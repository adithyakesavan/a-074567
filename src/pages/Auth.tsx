  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabase.auth.signUp({
        ...
        options: {
          data: {
            username: username || email.split('@')[0], // Use email name part if no username provided
          },
        },
      });
      ...
      // For development, we can redirect directly to dashboard
      navigate('/dashboard');
    } catch (error: any) {
      toast({
      ...
      <div className="glass-card p-8 w-full max-w-md">
        ...
        <Tabs defaultValue="login" className="w-full">
          ...
          <TabsContent value="register">
            <form onSubmit={handleSignUp} className="space-y-6">
              <div className="space-y-2">
                <Label htmlFor="username-register">Username (optional)</Label>
                <Input
        ...
        <div className="mt-6 text-center">
          <p className="text-gray-400">
            <Button variant="link" className="text-blue-400 p-0" onClick={() => navigate('/')}>
              Back to Home
            </Button>
          </p>
        </div>
